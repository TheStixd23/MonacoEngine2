...
BaseApp::init() {
	...
	//hr = m_vertexBuffer.init(m_device, Espada[0], D3D11_BIND_VERTEX_BUFFER);
	//
	//if (FAILED(hr)) {
	//	ERROR("Main", "InitDevice",
	...
	//
	//// Create index buffer
	//hr = m_indexBuffer.init(m_device, Espada[0], D3D11_BIND_INDEX_BUFFER);
	//
	//if (FAILED(hr)) {
	//	ERROR("Main", "InitDevice",
...
BaseApp::render() {
	...
	//m_deviceContext.DrawIndexed(Espada[0].m_numIndex, 0, 0);
