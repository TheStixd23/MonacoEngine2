...
BaseApp::init() {
    ...
    // 1. Inicializar SwapChain y Device
    ...
    // 2. Inicializar Render Target View
    ...
    // 3. Inicializar Depth Stencil Texture
    ...
    // 4. Inicializar Depth Stencil View
    ...
    // CARGA DEL MODELO
    // ---------------------------------------------------------
    // Asegúrate que "Espada.obj" está en la carpeta junto al ejecutable (.exe)
    ...
    // 9. Inicializar Buffers de Geometría (Vertex e Index)
    ...
    // Establecer topología
    m_deviceContext.IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

    // 10. Inicializar Constant Buffers
    ...
    // CARGA DE LA TEXTURA
    // ---------------------------------------------------------
    // Usamos ExtensionType::PNG gracias a la implementación de stb_image en Texture.cpp
    hr = m_textureCube.init(m_device, "crucible_baseColor", ExtensionType::PNG);

    if (FAILED(hr)) {
        ERROR("Main", "InitDevice",
            ("Failed to initialize texture 'crucible_baseColor'. HRESULT: " + std::to_string(hr)).c_str());
        return hr;
    }

    // 11. Inicializar Sampler State
...
{
    // Cálculo de tiempo para animación
    static float t = 0.0f;
    ...
    // Actualizar constantes de vista y proyección
    cbNeverChanges.mView = XMMatrixTranspose(m_View);
...
BaseApp::render() {
    ...
    // Configurar viewport y depth stencil
    ...
    // Asignar constantes a los shaders
    m_cbNeverChanges.render(m_deviceContext, 0, 1);
    m_cbChangeOnResize.render(m_deviceContext, 1, 1);
    m_cbChangesEveryFrame.render(m_deviceContext, 2, 1);
    m_cbChangesEveryFrame.render(m_deviceContext, 2, 1, true); // Bind también al PS
